...
{-@ measure mapSize @-}
{-@ mapSize :: xs:[Tree a] -> {v:[Pos] | len v = len xs} @-}
mapSize :: [Tree a] -> [Int]
mapSize [] = []
mapSize (x:xs) = size x : mapSize xs

{-@ measure sumPositives @-}
{-@ sumPositives :: xs:[Pos] -> {v:Nat | len xs <= v} @-}
sumPositives :: [Int] -> Int
sumPositives [] = 0
sumPositives (x:xs) = x + sumPositives xs

{-@ measure sumSizeList @-}
{-@ sumSizeList :: xs:[Tree a] -> {v:Nat | len xs <= v} @-}
sumSizeList :: [Tree a] -> Int
sumSizeList [] = 0
sumSizeList ts = sumPositives (mapSize ts)

-- sumSizeList (t:ts) == size t + sumSizeList ts

-- {-@ sumSizeLemma :: t:(Tree a) -> ts:[Tree a] -> {v:[Tree a] | v = (t:ts) && sumSizeList (t:ts) = size t + sumSizeList ts} @-}
-- sumSizeLemma :: Tree a -> [Tree a] -> [Tree a]
-- sumSizeLemma t ts = t:ts

{-@ measure heapSize @-}
{-@ heapSize :: h:(Heap a) -> Nat @-}
heapSize :: Heap a -> Int
...
-- {-@ eltsTree :: t:(Tree a) -> Set a / [size t] @-}
-- eltsTree :: (Ord a) => Tree a -> Set a
-- eltsTree (Node r x ts sz) =
--   assert (sz == 1 + sum (map size ts)) $
--   List.foldl' S.union (S.singleton x) (map eltsTree ts)
